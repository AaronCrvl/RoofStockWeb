...
function SideNav() {
  ...
  return (
    <div className="p-5 h-full">
      <span
        className="place-self-start text-4xl font-bold select-none hover:cursor-pointer"
        onClick={() => handleSideNavRedirect("/dashboard")}
      >
        RoofStock
      </span>
      <div className="mt-20 w-full">
        {[
          ...
          <div
            key={index}
            className={"mt-10 block hover:border-t-2 hover:shadow-xl hover:border-b-2 hover:border-gray-400 hover:cursor-pointer w-full hover:scale-125 transition delay-50 duration-300 ease-in-out "} // TODO: How can i put this shit on the bottom ? Damn
          >
            <div className={"flex rounded-lg p-2"}>
              <div>{item.icon}</div>
              <span
                key={index}
                className="ml-4 place-self-start block"
