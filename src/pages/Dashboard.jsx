...
const Dashboard = () => {
  ...
  const [stocksOverview, setStocksOverview] = useState(null);
  const [products, setProducts] = useState(null);
  const [productsGridView, setProductsGridView] = useState(null);
  ...
  useEffect(() => {
    ...
    if (stocks == null) {
      setInterval(() => {
        //TODO: const STOCKS_LIST = getStockByUser(userId);
        setStocks(STOCKS_LIST);
        setStocksOverview(STOCKS_LIST[0]);
      }, 2000);
    }
  }, [stocks, userId]);

  useEffect(() => {
    if (stocks != null && products == null) {
      setInterval(() => {
        //TODO: setProducts(getStockProducts(stocks[0].idEstoque));
        setProducts(PRODUCT_LIST);      
        setProductsGridView(PRODUCT_LIST);
      }, 2000);      
    }
  }, [products, stocks]);
  ...
  return (
    <Layout>
      <PageContainer.Root>
        ...
        <PageContainer.Body>
          {products === null || productsGridView === null || stocks === null ? (
            <div className="p-6 bg-white rounded-lg shadow text-gray-500">
              Carregando produtos e dados de dashboard...
            </div>
          ) : (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 mb-8">
                {[
                  ...
                  },
                  {
                    ...
                    icon: (
                      <ArrowDownIcon className="w-10 h-10 text-orange-500" />
                    ),
                  },
                  {
              ...
              <div className="flex flex-col sm:flex-row gap-4 bg-white p-4 rounded-lg items-stretch sm:items-center justify-between mb-4">
                ...
                <select className="p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 text-black">
                  ...
                  {["Alcolico", "Sem Alcool"].map((type, index) => (
                    <option key={index} value={type === "Alcolico" ? 0 : 1}>
                      {type}
                    </option>
                  ))}
                </select>
